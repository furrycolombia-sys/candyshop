...
function run(conn, cmd, sudoPass) {
  return new Promise((res, rej) => {
    const fullCmd = sudoPass
      ? `sudo -S sh -c ${JSON.stringify(cmd)}`
      : cmd;
    ...
    conn.exec(fullCmd, (err, stream) => {
      ...
      const timer = setTimeout(() => {
        stream.destroy();
        rej(new Error(`Command timed out after ${CMD_TIMEOUT_MS / 1000}s: ${cmd}`));
      }, CMD_TIMEOUT_MS);

      stream.on("error", (e) => { clearTimeout(timer); rej(e); });
      stream.on("data",  (d) => { out += d; });
      stream.stderr.on("data", (d) => { out += d; });
      stream.on("close", () => { clearTimeout(timer); res(out.trim()); });
...
async function start(conn, sudoPass) {
  await run(conn, "systemctl start cloudflared && systemctl enable cloudflared", sudoPass);
  return getStatus(conn, sudoPass);
}

async function stop(conn, sudoPass) {
  await run(conn, "systemctl stop cloudflared && systemctl disable cloudflared", sudoPass);
  return getStatus(conn, sudoPass);
}
...
async function printServerStatus(label, host, connectFn, sudoPass) {
  ...
  try {
    ...
    conn.end();
    console.log(status === "active" ? "[active]  <-- serving traffic" : `[${status}]`);
  } catch (e) {
...
function parseTunnelToken(token) {
  ...
  try {
    ...
    return { tunnelID: decoded["tunnel_id"], accountID: decoded["account_tag"] };
  } catch {
...
async function cmdConfigureIngress() {
  ...
  console.log("Ingress rules updated. Restarting cloudflared on GCP to apply...");
