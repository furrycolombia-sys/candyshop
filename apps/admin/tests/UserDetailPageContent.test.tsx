...
vi.mock("@/features/users/presentation/components/UserHeader", () => ({
  UserHeader: ({ user }: { user: { email: string } }) => (
    <div data-testid="user-header">{user.email}</div>
  ),
}));

vi.mock("@/features/users/presentation/components/TemplateButtons", () => ({
  TemplateButtons: ({
    onToggleTemplate,
    onReset,
  }: {
...
vi.mock("@/features/users/presentation/components/PermissionGroupCard", () => ({
  PermissionGroupCard: ({ groupKey }: { groupKey: string }) => (
    <div data-testid={`perm-group-${groupKey}`} />
  ),
}));

vi.mock("@/features/users/presentation/components/UserDelegatesCard", () => ({
